...
Item {
    ...
    DialogBox {
        id: ejectErrorDialog
        titleText: qsTr("Couldn't eject the disc")
        messageText: qsTr("Make sure no other applications are accessing the disc, and then try again.")

        buttons: [
            DialogBox.OkButton {
                destructive: true
                onClicked: ejectErrorDialog.close()
            }
        ]
    }
}
