...
pub struct IndividualArtistView {
    artists_name: String,
    ...
    artists_art: Option<Art>,
}

impl IndividualArtistView {
    pub fn new(
        ...
        let Artist::Ok {
            ...
            name: artists_name,
            art: artists_art,
        } = artists.read(cx).clone()
        else {
            panic!("Artists is not Ok");
        };

        cx.new(|cx| {
            let database_subscription =
                cx.observe_global::<Database>(move |_, cx: &mut Context<IndividualArtistView>| {
                    update_tracks_query(artists_id, cx);
                });

            update_tracks_query(artists_id, cx);

            IndividualArtistView {
                artists_name: artists_name.unwrap_or("Artists".to_string()),
                artists_art,
...
impl Render for IndividualArtistView {
    fn render(&mut self, window: &mut Window, cx: &mut Context<Self>) -> impl IntoElement {
        ...
        .child(
            grandstand("tracks-grandstand")
                .text(tr!(
                    "INDIVIDUAL_ARTISTS_TITLE",
                    "Tracks by {{artists}}",
                    artists:quote = self.artists_name,
                ))
                .pt(px(36.))
...
fn update_tracks_query(artists_id: u32, cx: &mut Context<IndividualArtistView>) {
    let database = cx.global::<Database>();
    let query = database.query_artist_tracks(artists_id);

    cx.spawn(
        async move |tracks_view: WeakEntity<IndividualArtistView>, cx: &mut AsyncApp| {
...
fn artists_background(art: Option<Art>, bg: Rgba) -> ArtistsBackground {
    ArtistsBackground {
...
impl IntoElement for ArtistsBackground {
    type Element = Self;
...
struct ArtistsBackgroundPrepaintState {
...
impl Element for ArtistsBackground {
    type RequestLayoutState = ();
    type PrepaintState = ArtistsBackgroundPrepaintState;

    fn id(&self) -> Option<ElementId> {
        Some("individual-artists-background".into())
    }
    ...
    fn prepaint(
        ...
        let mut prepaint_state = ArtistsBackgroundPrepaintState {
            background_art: None,
...
impl Styled for ArtistsBackground {
    fn style(&mut self) -> &mut StyleRefinement {
        &mut self.style
    }
}

impl ParentElement for ArtistsBackground {
